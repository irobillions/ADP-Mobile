...
const basePath = environment.api_url + '/orders';
...
export class OrderService {
    ...
    getAllUserOrders() {
        return this.httpClient.get<OrderInterface[]>(`${basePath}?userId=${this.user.id}`, this.HttpOptions)
            .pipe(
                map( response => {
    ...
    getSpecificOrder(id: number) {
        return this.httpClient.get<OrderInterface>(`${basePath}/${id}?_embed=orderItems`, this.HttpOptions)
            .pipe(
                map( response => {
                    console.log(response);
                    return response;
