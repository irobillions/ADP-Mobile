...
const basePathPr = environment.api_url + "/clients/products";
const basePathcarM = environment.api_url + "/clients/brands";
...
export class ProductService {
  ...
  getAllProductForSpecificCategory(name: string) {
    return this.httpClient
      .get<any[]>(`${basePathPr}?categories=${name}`, this.HttpOptions)
      .pipe(
        map((response) => {
  ...
  getSingleProduct(id: number) {
    return this.httpClient.get(`${basePathPa}/${id}`, this.HttpOptions).pipe(
      tap((response) => {
  ...
  getCarModelsDetails(id: number) {
    return this.httpClient.get(`${basePathcarM}/${id}`, this.HttpOptions).pipe(
      tap((response) => {
        console.log(response);
      }),
      catchError(this.authService.errorHandler)
