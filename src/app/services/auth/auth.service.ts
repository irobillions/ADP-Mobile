...
export class AuthService {
  ...
  errorHandler(error: HttpErrorResponse) {
    if (error.error instanceof ErrorEvent) {
      ...
      console.error(error);
      console.error(
        `bad auth response: ${error.status}: ${
          error.statusText
  ...
  login(email: string, password: string) {
    ...
    return this.httpClient
      .post<LoginResponse>(LOGIN_API_ROUTE, { email, password })
      .pipe(
        map((response) => {
  ...
  getAccessToken(): Observable<string> {
    ...
    if (!this.jwt.isTokenExpired(accessToken)) {
      ...
      return this.httpClient
        .post<RefreshResponse>(REFRESH_API_ROUTE, { email, password }, opts)
        .pipe(
          map((response) => {
            localStorage.setItem("accessToken", response.accessToken);
  ...
  isAuthenticated(): boolean {
    return (
      ...
      this.jwt.isTokenExpired(localStorage.getItem("refreshToken")) === false
    );
  }
